...
#	$(RUN) gen-doc -d $(DOCDIR) $(SOURCE_SCHEMA_PATH)
	$(RUN) gen-markdown -I $(DOCDIR)/index.md -d $(DOCDIR) $(SOURCE_SCHEMA_PATH)
...
git-init:
	git init
git-add: .cruft.json
	git add .gitignore .github .cruft.json Makefile LICENSE *.md examples utils about.yaml mkdocs.yml poetry.lock project.Makefile pyproject.toml src/human_cell_atlas/schema/*yaml src/*/datamodel/*py src/data src/docs tests
